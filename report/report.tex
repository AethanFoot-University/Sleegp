...
% Display a requirements section as a table, with a list of requirements formatted as rows.
%
% The body should contain calls to \req, \req*, \subreq, and/or \subreq*, to format specific
% requirements as rows.
%
% Arguments:
% 1: The ID of the requirements section
\DeclareDocumentEnvironment{reqsection}{m}{%
  % Command definitions
	\reqsectiondecls{}
  ...
  \begin{longtable}{p{.07\textwidth} p{.43\textwidth} p{.40\textwidth}}
    ...
    \toprule
    \multicolumn{3}{l}{%
      \textbf{\pgfkeysvalueof{/reqsec/#1/number}%
        % Insert a label, for this non-starred version of the command
        \phantomsection\manuallabel{reqsec:#1}{\pgfkeysvalueof{/reqsec/#1/number}}%
        : \pgfkeysvalueof{/reqsec/#1/title}}} \\
    \midrule
...
% Display a requirements section as a table, with a list of requirements formatted as rows, without
% inserting a label for the requirements section itself.
%
% The body should contain calls to \req, \req*, \subreq, and/or \subreq*, to format specific
% requirements as rows.
%
...
\DeclareDocumentEnvironment{reqsection*}{m}{%
  ...
  \begin{longtable}{p{.07\textwidth} p{.43\textwidth} p{.40\textwidth}}
    ...
    \toprule
    \multicolumn{3}{l}{%
      \textbf{\pgfkeysvalueof{/reqsec/#1/number}: \pgfkeysvalueof{/reqsec/#1/title}}} \\
    \midrule
    % Display a header row.
    \textbf{ID} & \textbf{Requirement} & \textbf{Details} \\
    \midrule
